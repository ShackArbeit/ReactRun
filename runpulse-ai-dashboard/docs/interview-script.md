...
> 整個專案刻意對應 JD 的幾個核心要求：前端架構設計、效能優化、API 串接、GitHub Actions CI/CD，以及善用 AI 工具。  
...
- Bundle 優化：ECharts 改用 modular import（`echarts/core`）tree-shaking，DashboardPage chunk 從 1,041 kB 降到 13 kB；MSW 改為 dev-only 動態 import，消除 production 的 270 kB browser bundle；vendor chunks 拆分讓瀏覽器可以獨立 cache

### CI/CD
GitHub Actions：push/PR 自動跑 lint、typecheck、test、build。  
